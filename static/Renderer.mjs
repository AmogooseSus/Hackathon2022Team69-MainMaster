...
export default class Renderer{
  constructor(){
    ...
    const Mogi = [];
    const MogusParent = document.createElement("div");
    MogusParent.style.display = "block";
    MogusParent.style.position = "absolute";
    MogusParent.style.minWidth = "100vw";
    MogusParent.style.minHeight = "100vh";
    MogusParent.style.pointerEvents = "none";
    document.body.appendChild(MogusParent);
    for(let i = 0; i < 250; ++i){
      const Element = document.createElement("img");
      Element.crossorigin = "anonymous";
      Element.src = "mogus.png";
      Element.classList.add("Mogus");
      MogusParent.appendChild(Element);
      Mogi.push({
        "Element": Element,
        "Rotation": 150. + Math.random() * 300.,
        "XOffset": -5388. - Math.random() * 700. - i * 50.,
        "YOffset": -4140. - Math.random() * 700. - i * 50.,
        "XSpeed": 200. + Math.random() * 120. + 100. *  (1 / ((i + .5) / 3.)),
        "YSpeed": 160. + Math.random() * 80.,
        "Scale": 5000. + Math.random() * 3000.
      });
    }
    document.addEventListener("keydown", function(Event){
      if(Event.key === "a") MogusParent.style.display = "none";
      else if(Event.key === "m") MogusParent.style.display = "block";
    });
    ...
    void function Render() {
      ...
      for(const MogusID in Mogi) {
        const Mogus = Mogi[MogusID];
        Mogus.Element.style.transform = `scale(${window.innerWidth / (Mogus.Scale + iTime * 100.)}) translate(${Mogus.XOffset + iTime * Mogus.XSpeed}px, ${Mogus.YOffset + iTime * Mogus.YSpeed}px) rotate(${iTime * Mogus.Rotation}deg)`;
      }
